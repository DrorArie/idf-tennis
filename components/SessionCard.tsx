  ...
  const statusBadge = myRegistration ? (
    myRegistration.status === 'confirmed' ? (
      ...
      <span className="text-xs bg-yellow-100 text-yellow-700 px-2.5 py-1 rounded-full font-medium">
        Waitlist #{myRegistration.waitlist_position}
      </span>
    ) : (
  ...
  return (
    <div
      className={`bg-white rounded-2xl p-4 shadow-sm border-2 transition-colors ${
        isMyLevel ? 'border-blue-200' : 'border-gray-100'
      }`}
    >
      <div className="flex items-start justify-between mb-3">
        <div>
          <p className="text-xl font-bold text-gray-900">
            {TIME_LABEL[session.time_slot] ?? session.time_slot}
          </p>
          <p className="text-sm text-gray-500">
            {SKILL_LABEL[session.skill_level]}
            {isMyLevel && (
              <span className="ml-1.5 text-blue-500 font-medium">• Your level</span>
            )}
          </p>
        </div>
        {statusBadge}
      ...
      {!session.is_open ? (
        <div className="bg-gray-50 rounded-xl p-3 text-center">
          <p className="text-sm text-gray-400">Opens Tuesday at 12:00 PM</p>
        </div>
      ) : (
        <>
          <div className="mb-4">
            <div className="flex justify-between text-xs text-gray-500 mb-1.5">
              <span>
                {session.confirmed_count}/{session.capacity} spots taken
              </span>
            ...
            <div className="w-full bg-gray-100 rounded-full h-2">
              <div
                className={`h-2 rounded-full transition-all duration-300 ${
                  spotsLeft === 0 ? 'bg-red-400' : 'bg-blue-500'
                }`}
                style={{
                  width: `${(session.confirmed_count / session.capacity) * 100}%`,
                }}
              />
            </div>
          </div>
            ...
            <>
              ...
              {!isMyLevel && (
                <p className="text-xs text-gray-400 text-center mt-2">
                  This session is for {SKILL_LABEL[session.skill_level]} only
                </p>
              )}
            </>
          )}
